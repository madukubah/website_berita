...
class Slider extends User_Controller {
	const IMAGE_TYPE = 2;

	private $services = null;
    private $name = null;
    private $parent_page = '4dmin';
	private $current_page = '4dmin/slider/';
	...
	public function index()
	{
		...
		$table[ "rows" ] = $this->gallery_model->galleries( Slider::IMAGE_TYPE, $pagination['start_record'], $pagination['limit_per_page'] )->result();

		$table = $this->load->view('templates/tables/plain_table_image', $table, true);
		$this->data[ "contents" ] = $table;
		$add_menu = array(
			"name" => "Tambah Gambar Slider",
		...
		$this->data["block_header"] = "Gambar Slider";
		$this->data["header"] = "Gambar Slider";
	...
	public function add(  )
	{
        ...
        if ($this->form_validation->run() === TRUE )
        {
			...
			$filename = "Slider_".$data['name']."_".time();

...
class Iklan extends User_Controller {
	const IMAGE_TYPE = 4;
	private $services = null;
    private $name = null;
    private $parent_page = '4dmin';
	private $current_page = '4dmin/iklan/';
	...
	public function index()
	{
		...
		$table[ "rows" ] = $this->gallery_model->galleries( Iklan::IMAGE_TYPE, $pagination['start_record'], $pagination['limit_per_page'] )->result();
		...
		$this->data["block_header"] = "Iklan";
		$this->data["header"] = "Iklan";
	...
	public function add(  )
	{
        ...
        if ($this->form_validation->run() === TRUE )
        {
			...
			$filename = "Iklan_".$data['name']."_".time();
	...
	public function edit(  )
	{
        ...
        if ($this->form_validation->run() === TRUE )
        {
			...
			$this->upload->initialize($config);
			// echo var_dump( $_FILES['file_image']['name'] ); return;
